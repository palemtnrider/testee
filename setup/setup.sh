...
if [ "${TRACK}" == "wks-ssh" ]; then
    if [ "${SKIP_PREFLIGHT_CHECKS}" == "false" ]; then
        ...
        "${WK_EXECUTABLE}" setup check imagerepository "${CONFIG_FILE}" --verbose="${WKP_DEBUG}" || exit 1
    fi
fi
...
if [ -n "${GIT_URL-}" ]; then
    if [ ! -e "${GIT_DEPLOY_KEY}" ]; then
        ...
        echo "Upload this public key with read/write access to your git provider:" >&2
        echo "" >&2
        cat "${GIT_DEPLOY_KEY}.pub" >&2
        echo "" >&2
        echo "Then re-run: $ wk setup run" >&2
        echo "" >&2
        exit 1
    fi
...
if [ "${TRACK}" == "wks-footloose" ]; then
    ...
    git_commit_if_changed "Initial cluster configuration" \
        ...
        "${SCRIPT_DIR}/wks-controller.yaml" \
    ...
    eksctl utils write-kubeconfig "${CLUSTER_NAME}" -r "${REGION}"
else # unknown
